...
| grading_add_annotations | scanId; annotations[] | batch-add many annotations in one call (each needs kind + flat 4-number bbox) |
| grading_rotate | scanId; optional direction right/left | rotate the stored scan 90° to face forward; clears OCR/results/annotations (re-run grading_ocr) |
...
**`grading_add_annotation` bbox must be a FLAT 4-element array** `[x, y, w, h]` of numbers (numeric strings like `"540"` are accepted; do not nest it inside another array, and never pass an object). If a mark is misplaced, fix it by sending the same `annotationId` with corrected fields, or `remove:true` then re-add — **never** regenerate a separate image or use PIL. Every add/edit/remove applies directly to the right-hand preview and appears within ~4s (or on "刷新批改痕迹"); verify with `grading_preview`.
