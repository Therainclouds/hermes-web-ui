...
export function validateConsistency(v: any, canon: SceneCanon, body: string) {
  ...
  return { coverage, issues, passed: issues.length === 0, ...(repairReport ? { repairReport: true } : {}) }
}
...
export const CHECK_PROMPT = `独立验收已修订正文。逐一对照canon.events和原始rows/corrections，检查事件因果与时间、尝试与结果、角色/NPC对白归属、人物知识边界、stateBefore至stateAfter的伤势/地点/物品/关系、公开外貌，以及是否遗漏重要原文或把场外废话写进剧情。
输出 {coverage:[{eventId,paragraph}],issues:[{detail}]}。paragraph为manuscript按空行分段、去除首尾空白和空段之后的从0开始的段落编号，服务器从真实正文提取引用，不必重复抄写。也兼容{eventId,quote}，quote必须为正文完整逐字片段，不可改写。每个事件仅给一项；该段必须真正体现该事件，不能用无关段落占位。正文缺少某事件时将其eventId及具体缺失内容列入issues并省略对应coverage。
任何事实冲突、事件遗漏、重复灌水、无依据关键剧情都进入issues；完全通过才允许issues为空。允许不改变事实的环境氛围描写和保留原意的对白整理。不要改写正文。issues.detail应指出具体事件及需要修改的段落和内容，便于局部修订。`
