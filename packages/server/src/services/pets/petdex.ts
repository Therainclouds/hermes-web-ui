...
function normalizeManifest(value: unknown): PetdexManifest {
  ...
  const pets = Array.isArray(data.pets)
    ? data.pets.map(normalizePet).filter((pet): pet is PetdexPet => Boolean(pet))
    : []
  if (pets.length === 0) {
...
export async function fetchPetdexAsset(
  urlValue: string,
): Promise<{ buffer: Buffer; mime: string; maxAgeSeconds: number }> {
