...
interface StreamedText {
  agent_message_chunk: string
  agent_thought_chunk: string
}
...
export class DshAcpTurn {
  ...
  private receiveStream(frame: any) {
    ...
    if (frame.type === 'end') this.attempts.delete(frame.attemptId)
      ...
      (frame.type === 'text-delta' || frame.type === 'reasoning-delta') &&
  ...
  private receiveUpdate(update: any) {
    ...
    if (
      ...
      if (text.startsWith(remaining))
        update = { ...update, content: { ...update.content, text: text.slice(remaining.length) } }
    }
  ...
  async prompt(input: {
    ...
    const initialized = await this.request('initialize', {
      protocolVersion: 1,
      clientCapabilities: {},
      clientInfo: { name: 'hermes-web-ui', version: '1.0.0' },
    })
