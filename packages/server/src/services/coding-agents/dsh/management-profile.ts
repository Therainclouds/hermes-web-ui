...
export async function prepareDshManagementProfile(input: { command: string; sourceHome: string; rootDir: string }) {
  ...
  if (
    ...
    throw new Error('Invalid native DSH Web profile')
  }
  ...
  await writeFile(
    ...
    JSON.stringify({ ...manifest, name, version, dsh: { ...manifest.dsh, profile: { ...manifest.dsh.profile, patchReload: 'startup' } } }),
  )
  for (const [target, sourcePath] of [
    [join(directory, 'cordis.patch.yml'), join(source, 'cordis.patch.yml')],
    [join(input.rootDir, 'cordis.patch.yml'), join(input.sourceHome, 'cordis.patch.yml')],
  ] as const) {
  ...
  await writeFile(
    ...
    JSON.stringify({
      ...
      dsh: { client: { platform: 'web', immediately: true, inject: ['slots', 'layout', 'locale', 'theme'], external: ['react'] } },
    }),
  )
