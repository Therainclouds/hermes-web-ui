...
import { killOwnedProcessTree } from '../../../lib/process-tree'
...
export interface DshManagementLaunch {
...
export async function shutdownDshManagement() {
...
export class DshManagement {
  ...
  touch(generation: string) {
    if (
      !this.target ||
      this.target.generation !== generation ||
      this.child?.exitCode !== null ||
      this.child?.signalCode !== null
    ) {
      throw new DshPluginError(410, 'DSH_UI_EXPIRED', 'Reload the DSH configuration panel')
    }
    clearTimeout(this.idle)
  ...
  private async ensure() {
    ...
    const graph = await Promise.all(
      ['profiles/web/package.json', 'profiles/web/cordis.patch.yml', 'cordis.patch.yml'].map(path =>
        optionalDshFile(join(input.sourceHome, path)),
      ),
    )
    const fingerprint = createHash('sha256')
      .update(JSON.stringify([input.installationCommand, input.sourceHome, graph]))
      .digest('hex')
    if (
      this.target &&
      ...
      this.touch(this.target.generation)
      return this.target
    }
    ...
    if (previous?.pid && previous.exitCode === null && previous.signalCode === null) {
      ...
      await killOwnedProcessTree(previous.pid, () => previous.kill('SIGKILL'))
      await stopped
    }
    ...
    await mkdir(input.sourceHome, { recursive: true })
    this.root = await mkdtemp(join(directory, 'host-'))
    this.fingerprint = fingerprint
    const prepared = await prepareDshManagementProfile({
      command: input.installationCommand,
      sourceHome: input.sourceHome,
      rootDir: this.root,
    })
    const execution = this.host.commandExecution(input.installationCommand, [
      '--profile',
      prepared.profile,
      '--patch',
      prepared.patch,
    ...
    const child = spawn(execution.command, execution.args, {
      cwd: input.sourceHome,
      detached: process.platform !== 'win32',
      windowsHide: true,
      ...(execution.windowsVerbatimArguments ? { windowsVerbatimArguments: true } : {}),
      env: { ...(await this.host.commandEnv()), DSH_HOME: this.root, ELECTRON_RUN_AS_NODE: '1' },
      ...
      const target = await new Promise<DshUiTarget>((resolve, reject) => {
        ...
        function fail(reason: string) {
          if (!settled) {
            ...
            reject(new DshPluginError(503, 'DSH_UI_UNAVAILABLE', `Unable to start the DSH configuration runtime (${reason})`))
          }
        }
        ...
        child.once('close', code =>
          fail(`process exited before readiness${typeof code === 'number' ? `: ${code}` : ''}`),
        )
        ...
        child.stdout?.on('data', chunk => {
          ...
          void (async () => {
            ...
            if (exchange.status !== 303 || !cookie) {
              fail(`native authentication failed (HTTP ${exchange.status})`)
              return
            }
            ...
            if (!settled) {
              settled = true
              clearTimeout(timeout)
              resolve({ endpoint, cookie, generation: randomUUID() })
            }
          })().catch(error => {
            ...
            const detail = ['ECONNREFUSED', 'ECONNRESET', 'ETIMEDOUT', 'EHOSTUNREACH', 'ENETUNREACH'].includes(code)
              ? `: ${code}`
              : ''
