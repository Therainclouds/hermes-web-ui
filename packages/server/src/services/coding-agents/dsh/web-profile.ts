...
export function anchorDshPatch(content: string, filename: string) {
  ...
  const entries = (items: unknown[]) => {
    for (const item of items) {
      ...
      if (typeof name === 'string' && (isAbsolute(name) || /^\.\.?[/\\]/.test(name))) {
        item.set('name', pathToFileURL(resolve(dirname(filename), name)).href)
      }
  ...
  for (const item of (doc.contents as any).items)
    if (isMap(item) && isSeq(item.get('insert', true))) entries((item.get('insert', true) as any).items)
  return String(doc)
}
...
export async function prepareDshWebProfile(input: { command: string; sourceHome: string; rootDir: string; model?: string }) {
  ...
  const manifestText = await optionalDshFile(
    manifestPath,
  ...
  if (
    ...
    throw new Error('DSH Web profile must declare the native base and Web bundles')
  }
  ...
  const generation = createHash('sha256')
    .update(JSON.stringify([installation, manifestText, profilePatch, homePatch, layers, [...dependencies]]))
    .digest('hex')
    .slice(0, 16)
    ...
    JSON.stringify(
      {
        ...
        version: '0.0.0',
        private: true,
        dsh: { profile: { bundles: [...bundles, '@deepseek-ai/dsh-acp-app'], patchReload: 'startup' } },
      },
      null,
      2,
    ),
  )
  await writeFile(join(directory, 'cordis.patch.yml'), anchorDshPatch(profilePatch, join(sourceProfile, 'cordis.patch.yml')), {
    mode: 0o600,
  })
        ...
        insert: [
          {
            ...
            inject: ['acpAppStartup', 'agentPresets', 'agentDefaultModel', 'settings'],
            config: {},
          },
        ],
      },
  ...
  const modelConfig = parseDocument(
  ...
  await writeFile(
    ...
    JSON.stringify({ sourceHome: input.sourceHome, sourceProfile: 'web', generation, browserRuntime: false, excludedWebRows: disabled }, null, 2),
  )
...
export function dshPresetSourceConfig(layers: string[], sourceHome: string): YAMLMap {
  ...
  for (const text of layers) {
    ...
    for (const patch of (doc.contents as any).items) {
      ...
      const candidates = [
        patch,
        ...(isSeq(patch.get('insert', true)) ? (patch.get('insert', true) as any).items : []),
      ]
      for (const row of candidates)
        if (isMap(row) && row.get('id') === 'agent-presets' && isMap(row.get('config', true)))
          presetConfig = (row.get('config', true) as unknown as YAMLMap).clone() as YAMLMap
    }
  }
  ...
  if (presetConfig.get('includeUserRoot') !== false)
    configuredRoots.add({ path: join(sourceHome, '.agent-presets'), trust: 'user' })
