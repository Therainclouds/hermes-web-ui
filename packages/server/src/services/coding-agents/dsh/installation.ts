...
export async function dshInstallation(command: string) {
  ...
  try {
    directory = dirname(await realpath(command))
  } catch {
    throw new DshPluginError(503, 'DSH_DEPENDENCY_UNAVAILABLE', 'DSH is not installed')
  }
  for (;;) {
    for (const candidate of [join(directory, 'package.json'), join(directory, 'node_modules/@deepseek-ai/dsh/package.json')]) {
      try {
        if (JSON.parse(await readFile(candidate, 'utf8')).name === '@deepseek-ai/dsh') return candidate
      } catch {
        /* keep scanning */
      }
    }
...
export async function dshPackageDirectory(name: string, anchors: string[]) {
  ...
  throw new DshPluginError(
    422,
    'DSH_DEPENDENCY_UNAVAILABLE',
    `Cannot resolve Web profile dependency ${name}; install its dependencies in the source Web profile`,
  )
}
