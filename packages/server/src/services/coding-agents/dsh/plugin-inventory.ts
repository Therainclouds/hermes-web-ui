  ...
  const scan = (sequence: unknown, parents: string[], inherited: boolean | 'conditional') => {
    ...
    for (const [index, row] of sequence.items.entries()) {
      ...
      const own =
        disabled === undefined
          ? true
          : isScalar(disabled) && !disabled.tag && typeof disabled.value === 'boolean'
            ? !disabled.value
            : 'conditional'
      const enabled =
        inherited === false || own === false
          ? false
          : inherited === 'conditional' || own === 'conditional'
            ? 'conditional'
            : true
      if (row.get('group') === true) scan(row.get('config', true), [...parents, entryId], enabled)
        ...
        entries.push({
          entryId,
          moduleName: String(row.get('name')),
...
export async function readNativeDshPluginInventory(command: string, sourceHome: string) {
  ...
  try {
    packagePath = createRequire(installation).resolve('@deepseek-ai/dsh-agent-presets/package.json')
  } catch {
    throw new DshPluginError(422, 'DSH_CAPABILITY_UNSUPPORTED', 'This DSH installation does not expose native presets')
  }
  ...
  await Promise.all(
    inventory.presets.flatMap(preset =>
      preset.entries.map(async entry => {
        if (!cache.has(entry.moduleName)) {
          cache.set(
            entry.moduleName,
            readDshPluginMetadata(entry.moduleName, [installation, join(sourceHome, 'profiles/web/package.json')]),
          )
        }
  ...
  const packages = await Promise.all(
    Object.entries(manifest.dependencies || {}).map(async ([name, requested]) => {
      ...
      try {
        ...
        entry.title = typeof pkg.displayName === 'string' ? pkg.displayName : String(pkg.name || name)
        entry.description = typeof pkg.description === 'string' ? pkg.description : ''
        entry.version = String(pkg.version || '')
        entry.containsBrowserPart = !!pkg.dsh?.client
      } catch {
        entry.error = 'DSH_DEPENDENCY_UNAVAILABLE'
      }
      return entry
    }),
  )
...
export async function readNativeDshPresetRoots(packagePath: string, sourceHome: string) {
  ...
  const roots = [
    { path: join(dirname(packagePath), 'presets'), trust: 'system' as const },
    { path: join(sourceHome, '.agent-presets'), trust: 'user' as const },
  ]
  const presets: DshNativePreset[] = []
  for (const root of roots) {
    let directories
    try {
      directories = await readdir(root.path, { withFileTypes: true })
    } catch (error: any) {
    ...
    for (const directory of directories) {
      ...
      try {
        if (!within(canonical, await realpath(path)) || !(await stat(path)).isDirectory()) continue
      } catch {
        continue
      }
      const sourcePath = join(path, 'agent.cordis.yml')
      const preset: DshNativePreset & { order: number } = {
        id: directory.name,
        name: directory.name,
        description: '',
        trust: root.trust,
        sourcePath,
        isDefault: directory.name === defaultPreset,
        entries: [],
      ...
      try {
        ...
        for (const key of ['name', 'description'] as const)
          if (typeof metadata?.get(key) === 'string') preset[key] = String(metadata.get(key))
  ...
  return {
    ...
    discovery: 'shipped-and-user-roots' as const,
    presets,
  }
}
