...
export function dshUiDocument(html: string, mount: string) {
  ...
  return html
    .replace(/\b(src|href)=(['"])\/(?!\/)([^'"]*)\2/g, (_, attr, quote, path) => `${attr}=${quote}${mount}${path}${quote}`)
    .replace(
      /<head([^>]*)>/i,
      `<head$1><base href="${mount}"><script>${bootstrap}</script>`,
    )
    .replace(
      '</head>',
      `<style>
...
function frameTransport(mount: string) {
  ...
  const heartbeat = setInterval(() => {
    void nativeFetch(mount + '__studio_ping')
      .then(response => {
  ...
  window.fetch = (input, init) =>
    nativeFetch(input instanceof Request ? new Request(map(input.url), input) : map(input), init)
  ...
  ;(window as any).__DSH_TRANSPORT__ = {
    loadBundle(url: string) {
      return new Promise<void>((resolve, reject) => {
        ...
        script.onerror = () => reject(new Error('DSH plugin resource unavailable'))
