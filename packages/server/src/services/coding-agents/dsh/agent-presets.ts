...
export class DshAgentPresetService {
  ...
  private async call<T>(method: string, args: Record<string, unknown> = {}): Promise<T> {
    ...
    try {
      ...
      throw new DshPluginError(502, 'DSH_PRESET_UNAVAILABLE', 'Unable to reach the DSH preset service')
    }
        ...
        code === 'agent-preset/not-found'
          ...
          : code === 'agent-preset/read-only'
            ...
            : code === 'agent-preset/invalid'
              ? 422
              : 502
      throw new DshPluginError(
        status,
        'DSH_PRESET_OPERATION_FAILED',
        message.result.error?.message || 'DSH preset operation failed',
      )
    }
  ...
  private id(value: unknown): string {
    if (typeof value !== 'string' || !/^[a-z0-9][a-z0-9-]*$/.test(value) || value.length > 200) {
      throw new DshPluginError(400, 'DSH_PRESET_INVALID', 'Invalid preset identifier')
    }
    return value
  }

  list() {
    return this.call<DshAgentPresets>('agentPresets/list')
  }

  async choices() {
    const { presets } = await this.list()
    return {
      presets: presets.map(({ id, name, description, isDefault, broken }) => ({
        id,
        name,
        description,
        isDefault,
  ...
  async read(id: unknown) {
    return this.call<{ agentPreset: string; content: string; name?: string; trust: 'system' | 'user' }>(
      'agentPresets/read',
      { agentPreset: this.id(id) },
    )
  }
  ...
  async copy(body: unknown) {
    ...
    if (
      ...
      throw new DshPluginError(400, 'DSH_PRESET_INVALID', 'Invalid preset copy request')
    await this.call('agentPresets/copy', {
      from: this.id(value.from),
      id: this.id(value.id),
  ...
  async openLocation(id: unknown) {
    return this.call<{ path?: string; opened: boolean }>('settings/openAgentPresetDirectory', { agentPreset: this.id(id) })
  }
}
