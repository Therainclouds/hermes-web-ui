...
export function createDshHost(host: DshHostCommands) {
  ...
  async function executeDshPluginCommand(home: string, args: string[], signal: AbortSignal): Promise<void> {
    ...
    await new Promise<void>((resolve, reject) => {
      ...
      child.once('error', () => {
        cleanup()
        reject(new DshPluginError(503, 'DSH_DEPENDENCY_UNAVAILABLE', 'DSH or its package manager is unavailable'))
      })
      child.once('close', code => {
        cleanup()
        if (signal.aborted)
          reject(new DshPluginError(503, 'DSH_OPERATION_INTERRUPTED', 'Plugin operation interrupted'))
        else if (timedOut) reject(new DshPluginError(503, 'DSH_OPERATION_TIMEOUT', 'Plugin operation timed out'))
        else if (code === 0) resolve()
        else if (/pnpm not found|ENOENT/.test(stderr))
          reject(new DshPluginError(503, 'DSH_DEPENDENCY_UNAVAILABLE', 'Install DSH and pnpm before managing plugins'))
        else
          reject(
            new DshPluginError(
              422,
              'DSH_PLUGIN_OPERATION_FAILED',
              'Web plugin operation failed; reload the list to inspect its current state',
            ),
          )
      })
    })
  ...
  const management = new DshManagement({
    runtimeInput,
    commandEnv: host.commandEnv,
    commandExecution: host.commandExecution,
  })
  ...
  return {
    getNativeDshPluginInventory,
    executeDshPluginCommand,
    runtimeInput,
    management,
    ui,
    changePlugins,
    presets: new DshAgentPresetService(management),
  }
}
