...
import { isSharedCodingAgentSkill } from '../shared-skills'
...
export function validateDshSkill(content: string): { name: string; description: string } {
  ...
  let data: any
  try {
    data = frontmatter ? load(frontmatter[1]) : null
  } catch {
    /* reported below */
  }
  if (
    ...
    throw Object.assign(new Error('DSH skills require YAML frontmatter with a kebab-case name and a description'), {
      status: 400,
    })
  }
    ...
    for (const entry of entries.sort()) {
      ...
      try {
        ...
        skills.set(name, {
          ...
          enabled: true,
          source: 'local',
          readonly: await isSharedCodingAgentSkill(file.path),
        })
      } catch {
        /* DSH also omits malformed skill definitions. */
      }
    }
  }
  return skills.size
    ? [{ name: 'misc', description: '', skills: [...skills.values()].sort((a, b) => a.name.localeCompare(b.name)) }]
    : []
}
