...
export async function writeDshAcpAdapter(installation: string, destination: string) {
  ...
  replace(
    ...
    'meta: { cwd: options.cwd, agentPreset: options.agentPreset },',
  )
  replace(
    'preset mounting',
    'await mountAcpMcpServers(agentCtx, options.mcpServers, options.cwd);',
    'await mountAcpMcpServers(agentCtx, options.mcpServers, options.cwd);\n\t\t\t\tawait ctx.agentPresets.mount(agentCtx, options.agentPreset);',
    2,
  )
  replace(
  ...
  replace(
    'resume-session preset selection',
    'agentOptions: agentOptions(config),\n\t\t\t\t\t\tfallbackSelection:',
    'agentOptions: agentOptions(config),\n\t\t\t\t\t\tagentPreset,\n\t\t\t\t\t\tfallbackSelection:',
  )
  replace(
  ...
  source = source.replace(
    ...
    (match, specifier: string) =>
      specifier.startsWith('node:') ? match : `from ${JSON.stringify(pathToFileURL(require.resolve(specifier)).href)}`,
  )
  const license = await readFile(join(directory, 'LICENSE'), 'utf8').catch(() =>
    readFile(join(dirname(installation), 'LICENSE'), 'utf8'),
  )
  await writeFile(destination + '.LICENSE', license)
  await writeFile(
    destination,
    `// Studio DSH ACP adapter revision ${DSH_ACP_ADAPTER_REVISION}; upstream ${manifest.version} (MIT); source sha256 ${sourceHash}.\n` + source,
    { mode: 0o600 },
  )
}
