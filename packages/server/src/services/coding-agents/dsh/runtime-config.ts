...
export const DSH_MODEL_PROVIDER = 'hermes-web-ui'
export const DSH_API_KEY_ENV = 'HERMES_DSH_API_KEY'

export interface DshContextPolicy {
...
export async function prepareDshRuntime(input: {
  ...
  contextWindow?: number
  outputLimit?: number
  contextPolicy?: DshContextPolicy
  imageInput?: boolean
  ...
  const web = input.installationCommand
    ? await prepareDshWebProfile({ command: input.installationCommand, ...input })
    : undefined
  const read = async (name: string) => {
    try {
      return await readFile(join(input.sourceHome, name), 'utf8')
    } catch (error: any) {
              ...
              models: [
                {
                  ...
                  maxTokens: input.outputLimit || 8192,
  ...
  return {
    ...
    env: {
      DSH_HOME: input.rootDir,
      DSH_PERMISSION_MODE: 'danger-full-access',
      ...(input.launchPath ? { PATH: input.launchPath } : {}),
    },
    files: ['settings.yaml', 'cordis.patch.yml', 'AGENTS.md', 'studio.patch.yml'].map(path => ({
      key: path,
      path,
      absolutePath: join(input.rootDir, path),
    })),
  }
}
