...
export class DshUiGateway {
  ...
  async create(userToken: string) {
    ...
    if (user?.role !== 'super_admin') {
      throw new DshPluginError(403, 'DSH_UI_FORBIDDEN', 'Super administrator required')
    }
    for (const [id, frame] of this.frames) if (frame.expires < Date.now()) this.remove(id, frame.userToken)
    if (this.frames.size >= 32) {
      throw new DshPluginError(409, 'DSH_UI_BUSY', 'Close an existing DSH configuration panel')
    }
  ...
  handlesUpgrade(req: IncomingMessage) {
  ...
  attach(servers: Server[]) {
    ...
    const upgrade = async (req: IncomingMessage, socket: Duplex, head: Buffer) => {
      ...
      try {
        ...
        if (
          ...
          (await authenticateUserToken(frame.userToken))?.role !== 'super_admin'
        )
          return reject()
        ...
        const headers: Record<string, string> = {
          connection: 'Upgrade',
          upgrade: 'websocket',
          cookie: frame.target.cookie,
          origin: frame.target.endpoint,
        }
        ...
        const revalidate = setInterval(() => {
          void authenticateUserToken(frame.userToken).then(user => {
            if (frame.expires < Date.now() || user?.role !== 'super_admin' || !this.frames.has(match![1])) socket.destroy()
        ...
        socket.once('close', () => {
          ...
          frame.connections.delete(socket)
        })
    ...
    return () => {
      for (const server of servers) server.off('upgrade', upgrade)
  ...
  middleware = async (ctx: Context, next: Next) => {
    ...
    ctx.set(
      ...
      "default-src 'self'; base-uri 'self'; object-src 'none'; frame-ancestors 'self'; script-src 'self' 'unsafe-inline' 'unsafe-eval'; style-src 'self' 'unsafe-inline'; img-src 'self' data: blob: https:; font-src 'self' data:; connect-src 'self' http: https: ws: wss:",
    )
    if (
      !frame ||
      frame.expires < Date.now() ||
      ...
      ctx.status = 401
      ctx.body = 'DSH configuration session expired'
      return
    }
    if (ctx.get('Origin')) {
      try {
        if (new URL(ctx.get('Origin')).host !== ctx.host) {
          ctx.status = 403
          return
        }
      } catch {
    ...
    try {
      this.management.touch(frame.target.generation)
      if (match![2] === '__studio_ping' && ctx.method === 'GET') {
        ctx.status = 204
        return
      }
