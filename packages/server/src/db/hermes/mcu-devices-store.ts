...
export function listMcuDevices(): McuDeviceRecord[] {
  ...
  const rows = db
    .prepare(`SELECT * FROM ${MCU_DEVICES_TABLE} ORDER BY created_at DESC, id DESC`)
    .all() as unknown as StoredMcuDeviceRow[]
...
export function getMcuDevice(id: number): McuDeviceRecord | null {
  ...
  const row = db
    .prepare(`SELECT * FROM ${MCU_DEVICES_TABLE} WHERE id = ?`)
    .get(id) as unknown as StoredMcuDeviceRow | undefined
  return row ? rowToRecord(row) : null
}
...
export function createMcuDevice(input: {
  ...
  try {
    const result = db
      .prepare(
        `INSERT INTO ${MCU_DEVICES_TABLE} (name, device_code, is_official, created_at) VALUES (?, ?, ?, ?)`,
      )
    ...
    const row = db
      .prepare(`SELECT * FROM ${MCU_DEVICES_TABLE} WHERE id = ?`)
      .get(id) as unknown as StoredMcuDeviceRow
    return rowToRecord(row)
  } catch (error: any) {
...
export function updateMcuDeviceName(id: number, name: string): McuDeviceRecord {
  ...
  db.prepare(`UPDATE ${MCU_DEVICES_TABLE} SET name = ? WHERE id = ?`).run(
    normalizedName || existing.device_code,
    id,
  )
  const row = db
    .prepare(`SELECT * FROM ${MCU_DEVICES_TABLE} WHERE id = ?`)
