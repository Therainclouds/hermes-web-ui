...
新增「长文小说化」：选择目标 1/2/4/6 万中文字符，使用当前全部转写和公开外貌，在后台完成分块提取、章节规划、逐场景写作与审核，再组装全书。显示覆盖进度，支持取消、关闭页面后继续、重启后断点续写。章节数可提示 2–24，自动按可用场景调整。篇幅是目标，素材不足会提示；不通过原来的聊天标签页/MCP 一次保存路径。**断点续写只生成缺失的部分**：已有的转写块、事实账本和场景初稿会复用，不会被提示词改动重新生成。工作台「生成产物」里可以对单个产物执行**删除**或**删除并重新生成**（旧版本保留在历史里）；删除后再次继续只会重建缺失或已过期的部分，若重建出的内容与原来不同，依赖它的下游产物会自动重算（不再需要手工一并删除）。每个产物记录它所消费内容的指纹：改提示词不会重买已付的断点，改事实账本/原文/章节方向则会重建受影响的正文。模型输入超限时会自动压缩上下文并对滚动记忆做模型摘要（结果按来源缓存复用），只有在保护性证据本身超限时才会明确报错而不丢弃原文。详见 [长文流水线与恢复说明](../../../../../docs/harness/trpg-long-novel.md)。

### 实时输出面板（LiveOutputStream）

工作台顶部的「AI 正在输出」面板（`LiveOutputStream.vue`）把每个进行中的产物按步骤类型渲染成 AI 工具调用风格的卡片，而不是直接吐原始 JSON：

- **步骤徽章**：解析 `step` 前缀（`review-*` / `revision-*` / `check-*` / `canon-*` / `write-*` / `plan-*` 等）配上对应颜色；同一步骤名显示 `label()`，并把内部 step id 渲染成等宽小标签。
- **正文 prose**：`body` 字段按 `\n\n` 切成衬线字体段落；`title + guide` 的章节骨架用标题 + 简介渲染。
- **可折叠结构化区**：连续性笔记 / 事件覆盖 / 待修复问题 / 可选建议 / 审阅提示 / 事实账本 / 状态变更 / 省略行都分别折叠起来，附计数徽章。
- **流式兼容**：JSON 不完整时（流式中途）自动回落 `<pre>` 原文，不丢任何字符；折叠区「`</>` 原始 JSON」始终可以展开查看底层数据。
- **状态指示**：跑任务时显示闪烁的光标，跑完自动消失；日 / 夜两套主题，RTL-safe logical CSS。

新增 i18n key：`trpg.harness.issues` / `trpg.harness.coveredRows` / `trpg.harness.rawSource` / `trpg.harness.toolBadge.*`。

### 长文流水线（2026-09 增量）

- **粘性断点**：提取块（`extract-*`）、事实账本（`canon-*`）、场景初稿（`write-*`）保存后不再随提示词变更作废，续写只重建真正缺失或过期的部分；改动章节 epoch 仍会重建对应初稿。
- **场景范围收紧**：审核与事实修订都改成场景范围——只接收本场景的 `rows` / `corrections` / `canon.events`，章节/全书层面的 `guide` / `book` 不再注入它们的提示词；`chapter.guide` 中超出本场景范围的事件、对白、身世、法术、物品与人物归属会被识别为越界并按整段删除处理。
- **`patch_paragraphs` 支持删除**：编辑器现在可以一次性删除多个段落（`delete: true` 或 `text: ""`），解决「只让改不让删」导致的死循环。
- **`paragraphsOf` 共享**：审核、编辑器读取与补丁应用走同一套段落切分（去空白段），避免索引偏移。
- **依赖指纹**：每个产物记录它所消费内容的指纹（`revisionPolicy()` 把写作 / 审核 / 编辑器 / 验收契约一起哈希）；改提示词不会重买已付的断点，改事实账本 / 原文 / 章节方向才会重建受影响的正文。`blocked` 状态被持久化，避免无变化的 resume 反复打模型。
- **租约 / 启动对账**：每个长篇任务持有一个 `lease.json`；拥有者进程消失后会被下一次启动对账标为 `interrupted`，并发进程遇到外租约会拒绝接管控该任务。
- **编辑轮次上限**：每个编辑决策的工具轮次上限从 6 提到 8；每个场景跨续写的固定提交轮次仍是 3。
- **失败清除**：成功的修订路径会清除上一步留下的 `job.failure`，避免状态条误显示。
