...
const REALTIME_SUPPLEMENT = [
  ...
  // 用户反馈：query_hermes_agent 的产物（含文件链接）在实时语音对话里用户
  // 拿不到——既不会出现在语音气泡里、也点不动下载链接，等于白做。改为
  // "用一句口语化的话告诉用户去工作区/文字对话页面看"，模型自然回退。
  '- 不要调用 query_hermes_agent：它的产物（文件链接 / 工作区写文件 / 长报告）在实时语音对话里用户拿不到。如果用户要做"生成文件 / 跑长任务"这类事，简短告诉用户"我这边语音回不来文件，请回文字对话页或到工作区继续"即可。',
].join('\n')
