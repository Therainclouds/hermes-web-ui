export interface ChatAgentAvatar {
  label: 'Hermes' | 'Ekko' | 'Claude' | 'Codex' | 'Pi' | 'DeepSeek Harness'
  src: string
}
...
const AGENT_AVATARS = {
  ...
  'dsh': { label: 'DeepSeek Harness', src: '/coding-agents/dsh.svg' },
...
export function chatSessionAgentAvatar(session?: ChatAgentSessionIdentity | null): ChatAgentAvatar {
  ...
  if (runtime === 'dsh' || runtime === 'deepseek-harness') return AGENT_AVATARS['dsh']
