...
const emptyState = computed(() => {
  ...
  const codingAgentId = session?.source === "global_agent"
    ? "ekko-agent"
    : session?.codingAgentId || (session?.agent === "codex" ? "codex" : session?.agent === "claude" ? "claude-code" : session?.agent === "deepseek" ? "deepseek-harness" : session?.agent === "ekko-agent" ? "ekko-agent" : undefined);
  ...
  if (codingAgentId === "deepseek-harness") {
    return {
      logo: "/coding-agents/deepseek-harness.svg",
