...
watch(
  ...
  () => emit('valid', !loading.value && !failed.value && !!current.value && !current.value.unavailable),
  { immediate: true },
)
...
<template>
  <div class="preset-field" data-testid="dsh-session-preset">
    ...
    <NSelect
      :input-props="{ id: inputId, 'aria-label': t('dshPresets.sessionMode') }"
      :value="selected"
      :options="options"
      :loading="loading"
      :disabled="props.disabled || loading || failed"
      :placeholder="t('dshPresets.selectMode')"
    ...
    <p v-else-if="!loading && !presets.some(preset => !preset.unavailable)" class="hint">
      {{ t('dshPresets.empty') }}
    </p>
  </div>
</template>
