...
const entries = computed(() =>
  (preset.value?.entries || []).filter(
    entry =>
      (!status.value || String(entry.configuredEnabled) === status.value) &&
      `${entry.entryId} ${entry.moduleName} ${entry.title || ''} ${entry.description || ''}`
        .toLowerCase()
        .includes(search.value.toLowerCase().trim()),
  ),
)
const options = computed(() =>
  (inventory.value?.presets || []).map(item => ({
    label: `${item.name} (${item.id}) · ${item.entries.length}`,
    value: item.id,
  })),
)
...
async function refresh() {
  ...
  try {
    ...
    if (!result.presets.some(item => item.id === selected.value)) {
      selected.value = result.presets.find(item => item.isDefault)?.id || result.presets[0]?.id || null
    }
  } catch {
    if (!disposed && version === sequence) failed.value = true
  } finally {
    if (!disposed && version === sequence) loading.value = false
  }
}
...
<template>
  <div class="native-plugins" data-testid="dsh-native-plugins">
    <div v-if="loading" class="plugins-loading-state">
      <NSpin />
    </div>
    ...
    <template v-else-if="inventory">
      ...
      <div class="summary-grid native-summary">
        <div class="summary-card">
          ...
          <strong data-testid="native-plugin-count">{{ summary.total }}</strong>
        </div>
        <div class="summary-card success">
          <span class="summary-label">{{ t('dshPlugins.enabled') }}</span>
          <strong>{{ summary.enabled }}</strong>
        </div>
        <div class="summary-card warning">
          <span class="summary-label">{{ t('dshPlugins.conditional') }}</span>
          <strong>{{ summary.conditional }}</strong>
        </div>
        <div class="summary-card error">
          <span class="summary-label">{{ t('dshPlugins.disabled') }}</span>
      ...
      <div class="filter-row native-filters">
        <NInput
          ...
          clearable
          :placeholder="t('dshPlugins.searchPlugins')"
          :input-props="{ 'aria-label': t('dshPlugins.searchPlugins') }"
        />
        <NSelect v-model:value="selected" :options="options" data-testid="native-preset-select" />
        <NSelect
          v-model:value="status"
          :options="statusOptions"
          :placeholder="t('plugins.statusTitle')"
          clearable
          data-testid="native-status-select"
        />
      </div>
      <template v-if="preset">
          ...
          <table class="plugins-table">
            ...
            <tbody>
              <tr
                ...
                :key="[...entry.groupPath, entry.entryId].join('/')"
                data-testid="native-plugin-entry"
              >
                <td>
                  <div class="plugin-name">
                    <strong>{{ entry.title || entry.entryId }}</strong>
                    <span v-if="entry.description">{{ entry.description }}</span>
                    <span>{{ entry.entryId }} · {{ entry.moduleName }}</span>
                  </div>
                  <div v-if="entry.groupPath.length" class="meta-line">{{ entry.groupPath.join(' / ') }}</div>
                </td>
                <td>
                  <NTag
                    size="small"
                    :type="entry.configuredEnabled === 'conditional' ? 'warning' : entry.configuredEnabled ? 'success' : 'error'"
                  >
                    {{ statusLabel(entry.configuredEnabled) }}
                  </NTag>
                </td>
                <td>
                  <NTag size="small" round>
                    {{ t(preset.trust === 'system' ? 'dshPlugins.shipped' : 'dshPlugins.userPreset') }}
                  </NTag>
                </td>
                <td>
                  <code class="path-cell" :title="preset.sourcePath">{{ preset.sourcePath }}</code>
                </td>
                <td>
                  <span class="muted">{{ t('plugins.managedElsewhere') }}</span>
                </td>
              </tr>
            </tbody>
          </table>
        ...
        <div class="metadata-panel">
          ...
          <span><code>{{ inventory.sourceHome }}</code></span>
          <span>DSH {{ inventory.packageVersion }}</span>
        </div>
      </template>
