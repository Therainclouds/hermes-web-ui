...
function select(value: string) {
  ...
  else if (configurationStale.value) {
    configurationStale.value = false
    void settings.value?.refresh()
  }
}
...
<template>
  <div class="plugins-view dsh-plugins" data-testid="dsh-plugins">
    <header class="page-header">
      <h2 class="header-title">{{ t('dshPlugins.title') }}</h2>
      <NButton size="small" quaternary @click="tab === 'configuration' ? settings?.refresh() : list?.refresh()">
        {{ t('mcp.refresh') }}
      </NButton>
    </header>
    <div class="plugins-content">
      <div ref="tabsElement" class="plugin-tabs" role="tablist" :aria-label="t('dshPlugins.title')" @keydown="keydown">
        <button
          ...
          aria-controls="dsh-config-panel"
          :tabindex="tab === 'configuration' ? 0 : -1"
          @click="select('configuration')"
        >
          {{ t('dshPlugins.configurationTab') }}
        </button>
        <button
          id="dsh-list-tab"
          role="tab"
          ...
          aria-controls="dsh-list-panel"
          :tabindex="tab === 'list' ? 0 : -1"
          @click="select('list')"
        >
          {{ t('dshPlugins.listTab') }}
        </button>
      </div>
      <div
        id="dsh-config-panel"
        v-show="tab === 'configuration'"
        role="tabpanel"
        aria-labelledby="dsh-config-tab"
      >
        <DshPluginSettingsPanel ref="settings" />
      </div>
      <div id="dsh-list-panel" v-show="tab === 'list'" role="tabpanel" aria-labelledby="dsh-list-tab">
        <DshNativePluginsPanel v-if="visitedList" ref="list" @changed="configurationStale = true" />
      </div>
    </div>
  </div>
</template>
