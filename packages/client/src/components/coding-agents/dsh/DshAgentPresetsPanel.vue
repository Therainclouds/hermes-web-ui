...
import {
  ...
  defaultDshAgentPreset,
  locateDshAgentPreset,
  type DshAgentPreset,
  type DshAgentPresets,
} from '@/api/coding-agents/dsh'
...
const error = ref('')
const copyError = ref('')
const source = ref<DshAgentPreset | null>(null)
const identifier = ref('')
const name = ref('')
const viewer = ref<{ title: string; content: string } | null>(null)

const validCopy = computed(
  () =>
    /^[a-z0-9][a-z0-9-]*$/.test(identifier.value) &&
    identifier.value.length <= 200 &&
    !roster.value.presets.some(row => row.id === identifier.value),
)
...
async function load() {
  ...
  error.value = ''
  try {
    roster.value = await listDshAgentPresets()
  } catch (err) {
...
async function mutate(operation: () => Promise<DshAgentPresets>) {
  ...
  error.value = ''
  try {
    roster.value = await operation()
  } catch (err) {
    error.value = message(err)
    await listDshAgentPresets()
      .then(value => {
        roster.value = value
      })
      .catch(() => {})
...
function beginCopy(row: DshAgentPreset) {
  source.value = row
  identifier.value = ''
  name.value = ''
  copyError.value = ''
...
async function copy() {
  ...
  copyError.value = ''
  try {
    roster.value = await copyDshAgentPreset({
      from: source.value.id,
      id: identifier.value,
      name: name.value,
    })
    source.value = null
  } catch (err) {
    copyError.value = message(err)
    await listDshAgentPresets()
      .then(value => {
        roster.value = value
      })
...
async function view(row: DshAgentPreset) {
  ...
  try {
    viewer.value = {
      ...
      content: (await readDshAgentPreset(row.id)).content,
    }
  } catch (err) {
    error.value = message(err)
  } finally {
...
async function locate(row: DshAgentPreset) {
  ...
  error.value = ''
  try {
    const result = await locateDshAgentPreset(row.id)
    if (!result.opened) viewer.value = { title: t('dshPresets.location'), content: result.path! }
  } catch (err) {
    error.value = message(err)
  } finally {
    busy.value = false
...
<template>
  <div class="plugins-view dsh-presets" data-testid="dsh-agent-presets">
    <header class="page-header">
      <h2 class="header-title">{{ t('dshPresets.title') }}</h2>
      <NButton size="small" quaternary :disabled="busy || loading" @click="load">{{ t('mcp.refresh') }}</NButton>
    </header>
    <div class="plugins-content">
      ...
      <div v-else-if="roster.presets.length" class="preset-grid">
        <section
          v-for="row in roster.presets"
          :key="row.id"
          class="preset-card"
          :class="{ selected: row.isDefault }"
          :data-testid="`dsh-preset-${row.id}`"
        >
          <div class="preset-heading">
            <h3>{{ row.name || row.id }}</h3>
            <NTag size="small" :bordered="false">
              {{ t(row.trust === 'system' ? 'dshPlugins.shipped' : 'dshPlugins.userPreset') }}
            </NTag>
            <NTag v-if="row.isDefault" size="small" type="success">{{ t('dshPresets.default') }}</NTag>
          </div>
          <code>{{ row.id }}</code>
          <p class="preset-description">{{ row.description }}</p>
          <NAlert v-if="row.broken" type="error">{{ row.broken }}</NAlert>
          <div class="preset-actions">
            <NButton size="small" :disabled="busy || !!row.broken" @click="view(row)">
              {{ t('dshPresets.view') }}
            </NButton>
            <NButton size="small" :disabled="busy || !roster.authorable || !!row.broken" @click="beginCopy(row)">
              {{ t('dshPresets.copy') }}
            </NButton>
            <NButton
              size="small"
              :disabled="busy || row.isDefault || !!row.broken"
              @click="mutate(() => defaultDshAgentPreset(row.id))"
            >
              {{ t('dshPresets.setDefault') }}
            </NButton>
            <NButton
              v-if="row.trust === 'user'"
              size="small"
              ...
              @click="locate(row)"
            >
              {{ t('dshPresets.location') }}
            </NButton>
            <NPopconfirm
              v-if="row.trust === 'user'"
              @positive-click="mutate(() => deleteDshAgentPreset(row.id))"
            >
              <template #trigger>
                <NButton size="small" type="error" ghost :disabled="busy">{{ t('dshPresets.delete') }}</NButton>
              </template>
              {{ t('dshPresets.deleteConfirm', { name: row.name || row.id }) }}
            </NPopconfirm>
          </div>
        </section>
      </div>
    ...
    <NModal
      :show="!!source"
      preset="card"
      :title="t('dshPresets.copyTitle', { name: source?.name || source?.id || '' })"
      class="preset-modal"
      :closable="!busy"
      :mask-closable="!busy"
      :close-on-esc="!busy"
      @update:show="value => { if (!value && !busy) source = null }"
    >
      <NAlert v-if="copyError" type="error">{{ copyError }}</NAlert>
      <form class="preset-form" @submit.prevent="copy">
        <label>
          {{ t('dshPresets.identifier') }}
          <NInput
            v-model:value="identifier"
            :disabled="busy"
            :maxlength="200"
            :input-props="{ 'aria-label': t('dshPresets.identifier') }"
          />
        </label>
        ...
        <label>
          ...
          <NInput
            ...
            :disabled="busy"
            :maxlength="200"
            :input-props="{ 'aria-label': t('dshPresets.name') }"
          />
        </label>
        <NButton attr-type="submit" type="primary" :disabled="!validCopy" :loading="busy">
          {{ t('dshPresets.create') }}
        </NButton>
      </form>
    </NModal>
    <NModal
      ...
      preset="card"
      :title="viewer?.title"
      class="preset-modal preset-viewer"
      @update:show="value => { if (!value) viewer = null }"
    >
