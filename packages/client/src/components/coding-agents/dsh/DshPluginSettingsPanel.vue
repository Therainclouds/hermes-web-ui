...
async function refresh() {
  ...
  try {
    ...
    if (disposed || current !== sequence) {
      void closeDshPluginUi(session.id).catch(() => {})
      return
    }
    const previous = id
    id = session.id
...
function syncTheme() {
  ...
  frame.value?.contentWindow?.postMessage(
    { type: 'studio-dsh-theme', theme: isDark.value ? 'dark' : 'light' },
...
function ready(event: MessageEvent) {
  if (!src.value || event.source !== frame.value?.contentWindow || event.origin !== new URL(src.value).origin)
    return
  if (event.data?.type === 'studio-dsh-ui-ready') {
    loading.value = false
    syncTheme()
  }
  if (event.data?.type === 'studio-dsh-ui-expired') {
    failed.value = true
    loading.value = false
  }
}
...
onUnmounted(() => {
  ...
  if (id) void closeDshPluginUi(id).catch(() => {})
})
...
<template>
  <div class="native-settings" data-testid="dsh-plugin-settings">
    ...
    <iframe
      ...
      ref="frame"
      :src="src"
      :title="t('dshPlugins.configurationTab')"
