...
When the login page reports that the IP is locked (HTTP 429/503), the same operations are available directly in the UI as two buttons â€” "Clear Login Lock" and "Reset Default Password" â€” protected by a single shared recovery password. By default the recovery password equals the shipped default admin password (`12345678`); override it with the `HERMES_WEB_UI_RECOVERY_PASSWORD` environment variable to use an independent value.
