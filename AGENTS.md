...
- **Phase (a) update invariants** (spec: [docs/harness/source-deploy-refactor.md](./docs/harness/source-deploy-refactor.md); violation of any of these reopens the v0.7.x incident class):
  ...
  - **Never pin `WEBUI_UPDATE_MANIFEST_URL` to a versioned `releases/vX.Y.Z/manifest.json` in device env**: a version-pinned manifest always answers successfully, so every update check "succeeds" with a frozen version — freshness green, empty `remoteError`, no update button, no error anywhere (device 6.6.6.73 missed the entire v0.8.x channel this way; the pin came from v0.8.6-era manual deploys and outlived them). A config override that always answers is indistinguishable from a healthy source, so this class of failure is silent by construction. The versioned manifest exists on **three hosts** — OSS object, GitHub Release asset, and the `release-manifests` branch archive — and a device pin may point at any one of them (6.6.6.73 pinned the branch archive; compat mirroring must cover all three, and `raw.githubusercontent.com` serves up to ~5 min stale after a commit). v0.8.9 adds `manifest_pinned_stale` detection (channel tip wins when the pinned version ≤ local; spec: `docs/knowledge/specs/task-12-auto-vaults-and-quotas.md` § Update-source hardening). Until then: after any manual deploy, check `/api/update/identity` → `manifestCache.version` ≥ the deployed version; if not, the env pin is the suspect.
