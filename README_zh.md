...
`clear-login-locks` 会删�?`${HERMES_WEB_UI_HOME:-~/.hermes-web-ui}/.login-lock.json`。如果服务正在运行，需要重启服务才能清理内存中的锁定状态。`reset-default-login` 会更�?Web UI 账户数据库；如果已存�?`admin` 用户，则会把密码重置�?`123456`，并启用为超级管理员账户�?
### 设置
...
| `BIND_HOST` | `0.0.0.0` | Web UI 绑定地址。如需 IPv6，可显式设置�?`::`�?|
| `HERMES_WEB_UI_HOME` | `~/.hermes-web-ui` | Web UI 数据目录，用于认�?token、登录凭据、日志、数据库和默认上传目录。兼容支�?`HERMES_WEBUI_STATE_DIR` 作为别名�?|
| `HERMES_WEBUI_STATE_DIR` | 未设�?| `HERMES_WEB_UI_HOME` 的兼容别名�?|
| `HERMES_WEB_UI_DISABLE_MCP_AUTOINJECT` | 未设�?| 关闭启动时向 Hermes profile 配置自动注入托管�?`hermes-studio` MCP server�?|
| `HERMES_WEB_UI_ALLOW_TRANSIENT_MCP_AUTOINJECT` | 未设�?| �?`HERMES_WEB_UI_HOME` 位于临时目录（例�?Version Preview runtime）时，仍允许托管 MCP 自动注入�?|
| `UPLOAD_DIR` | `$HERMES_WEB_UI_HOME/upload` | 覆盖上传根目录。文件会保存在按 Profile 隔离的子目录下�?|
| `CORS_ORIGINS` | 仅同 host | HTTP、Socket.IO、WebSocket 跨源 allowlist，支持逗号或空格分隔。只有明确需要旧�?wildcard CORS 时才设置�?`*`�?|
| `AUTH_TOKEN` | 自动生成 | 显式指定 bearer token。未设置时，Web UI 会在 `HERMES_WEB_UI_HOME` 下自动生成�?|
| `PROFILE` | `default` | 启动/默认 Hermes profile。运行时请求使用前端当前选择且当前账号有权限访问�?Profile�?|
| `LOG_LEVEL` | `info` | Server 日志级别�?|
| `BRIDGE_LOG_LEVEL` | `$LOG_LEVEL` �?`info` | Bridge 日志级别�?|
| `MAX_DOWNLOAD_SIZE` | `200MB` | 最大文件下载大小�?|
| `MAX_EDIT_SIZE` | `10MB` | 最大可编辑文件大小�?|
| `WORKSPACE_BASE` | 当前用户 Home 目录 | Workspace 浏览根目录�?|
| `HERMES_HOME` | 平台默认�?| Hermes 数据目录。Windows 使用 `%LOCALAPPDATA%\hermes`；macOS/Linux 使用 `~/.hermes`�?|
| `HERMES_BIN` | `hermes` | 自定�?Hermes CLI 二进制路径�?|
| `HERMES_AGENT_ROOT` | 自动发现 | 包含 `run_agent.py` �?Hermes Agent 源码目录�?|
| `HERMES_AGENT_BRIDGE_PYTHON` | 自动发现 | 用于启动 agent bridge �?Python 解释器�?|
| `HERMES_AGENT_BRIDGE_UV` | 自动发现 | 可用时用于启�?agent bridge �?`uv` 可执行文件�?|
| `UV` | 自动发现 | `uv` 可执行文�?fallback�?|
| `PYTHON` | 自动发现 | agent bridge �?Python 可执行文�?fallback�?|
| `HERMES_AGENT_BRIDGE_ENDPOINT` | 平台默认�?| Agent bridge broker endpoint。Windows 默认 `tcp://127.0.0.1:18765`；macOS/Linux 默认 `ipc:///tmp/hermes-agent-bridge.sock`�?|
...
| `HERMES_AGENT_BRIDGE_AUTO_RESTART` | 开�?| bridge broker 意外退出后是否自动重启；设�?`0`、`false`、`no` �?`off` 可关闭�?|
| `HERMES_AGENT_BRIDGE_RESTART_DELAY_MS` | `1000` | bridge 自动重启退避的基础延迟�?|
| `HERMES_AGENT_BRIDGE_PLATFORM` | `cli` | 传给 Hermes Agent �?platform 标识�?|
| `HERMES_AGENT_BRIDGE_WORKER_TRANSPORT` | 平台默认�?| Profile worker transport。设�?`tcp` 使用 loopback TCP；设�?`ipc`/`unix` 使用 Unix domain socket；默�?Windows TCP、macOS/Linux IPC�?|
| `HERMES_AGENT_BRIDGE_WORKER_PORT_BASE` | `18780` | TCP worker endpoint 起始端口�?|
| `HERMES_BRIDGE_PROVIDER` | profile/默认�?| bridge 运行时的 provider 覆盖�?|
| `HERMES_BRIDGE_TOOLSETS` | profile/默认�?| bridge 运行时的 toolset 覆盖�?|
| `HERMES_BRIDGE_MAX_TURNS` | profile/默认�?| bridge 运行时的最大轮数覆盖�?|
| `HERMES_BRIDGE_SUPPRESS_PLATFORM_HINT` | `cli` | 控制传给 Hermes Agent �?bridge platform hint suppression�?|
| `HERMES_OPENROUTER_APP_REFERER` | 未设�?| bridge 运行发送给 OpenRouter �?attribution referer；如需使用，请改成你们自己的公网站点�?|
| `HERMES_OPENROUTER_APP_TITLE` | `Hermes Web UI` | bridge 运行发送给 OpenRouter �?attribution title�?|
| `HERMES_OPENROUTER_APP_CATEGORIES` | `cli-agent,personal-agent` | bridge 运行发送给 OpenRouter �?attribution categories�?|
| `HERMES_WEB_UI_MANAGED_GATEWAY` | 默认开�?| 控制 Web UI 托管 Hermes gateway 进程；设�?`0`、`false`、`no` �?`off` 时改�?`hermes gateway start`�?|
| `HERMES_WEB_UI_DISABLE_GATEWAY_AUTOSTART` | 未设�?| 跳过启动时的 gateway 检�?自动启动；dashboard-only 部署中如果由其它服务管理 Hermes gateway，可设为 `1`、`true`、`yes` �?`on`�?|
| `HERMES_WEB_UI_DISABLE_SKILL_INJECTION` | 未设�?| 跳过启动时的内置 skill 注入；如果内�?skills �?Hermes Web UI 外部管理，可设为 `1`、`true`、`yes` �?`on`。启用注入时，Web UI 只更新自己此前安装的 skills 或内容完全相同的既有内置副本；本地修改和用户拥有的同�?skills 会跳过�?|
| `HERMES_WEB_UI_STOP_GATEWAYS_ON_SHUTDOWN` | 生产环境默认开�?| Web UI 关闭时是否同时停止托管的 gateway 进程；设�?`0` �?`false` 可让 gateway 分离运行�?|
| `GATEWAY_HOST` | `127.0.0.1` | �?gateway 兼容配置中写�?profile 的默�?gateway host�?|
| `HERMES_WEB_UI_PREVIEW_REPO` | package repository | Version Preview 使用�?GitHub 仓库�?|
| `HERMES_WEB_UI_PREVIEW_AGENT_BRIDGE_TRANSPORT` | 平台默认�?| Version Preview broker transport。设�?`tcp` 可让预览环境�?macOS/Linux 上也使用 loopback TCP；未设置时会跟随 `HERMES_AGENT_BRIDGE_WORKER_TRANSPORT=tcp`�?|
| `HERMES_WEB_UI_PREVIEW_AGENT_BRIDGE_ENDPOINT` | 隔离的预�?endpoint | 直接覆盖 Version Preview �?broker endpoint�?|
| `HERMES_WEB_UI_BACKEND_PORT` | `6060` | Vite dev proxy 使用的后端端口�?|
