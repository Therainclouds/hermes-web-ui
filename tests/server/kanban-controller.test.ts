...
describe('kanban controller', () => {
  ...
  it('lists and reads only attachments belonging to an authorized task', async () => {
    // path.resolve turns POSIX-style stored paths into cwd-relative Windows
    // paths, so anchor the attachment root through the native env override.
    const attachmentsRoot = join('/Users', 'tester', '.hermes', 'kanban', 'attachments')
    process.env.HERMES_KANBAN_ATTACHMENTS_ROOT = attachmentsRoot
    const storedPath = join(attachmentsRoot, 'task-1', 'report.html')
    mockGetTask.mockResolvedValue({
    ...
    expect(mockReadFile).toHaveBeenCalledWith(resolve(storedPath))
