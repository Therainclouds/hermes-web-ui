...
describe('ekko-agent context usage events', () => {
  ...
  it('loads frontend image blocks into ephemeral model content parts without storing base64', async () => {
    ...
    // Windows paths are JSON-escaped (backslashes doubled) inside stored content.
    expect(storedUserMessage?.content).toContain(process.platform === 'win32' ? JSON.stringify(imagePath).slice(1, -1) : imagePath)
