...
// Workspace paths are built with path.join at runtime, so expectations must
// use the same separators to stay portable across Windows/POSIX.
const { defaultProfileWorkspace, joinSegments } = vi.hoisted(() => {
  const sep = process.platform === 'win32' ? '\\' : '/'
  const joinSegments = (...segments: string[]) => segments.join(sep).replace(/^\//, sep)
  return {
    joinSegments,
    defaultProfileWorkspace: joinSegments('/tmp', 'hermes-bridge-final-context', 'default', 'workspace'),
  }
})
...
vi.mock('../../packages/server/src/services/hermes/hermes-profile', () => ({
  getProfileDir: (profile: string) => joinSegments('/tmp', 'hermes-bridge-final-context', profile || 'default'),
}))
...
describe('bridge run final context usage', () => {
  ...
  it('reopens an ended bridge session when starting a new run', async () => {
    getSessionMock.mockReturnValue({
      ...
      model: 'gpt-test',
      provider: 'openai',
      workspace: defaultProfileWorkspace,
